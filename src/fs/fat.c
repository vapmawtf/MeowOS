...
static int find_in_directory(FAT32_FS* fs, uint32_t dir_cluster, const char* name83,
...
static int resolve_parent_and_name(FAT32_FS* fs, const char* path, uint32_t* out_parent_cluster,
                                   char out_name83[11]) {
    char parent[128];
    const char* last = 0;
    size_t len;
...
static int fat32_find_free_entry_slot(FAT32_FS* fs, uint32_t dir_cluster, uint32_t* out_lba,
                                      uint32_t* out_off) {
    uint32_t cluster = dir_cluster;
...
static int fat32_create_entry(FAT32_FS* fs, uint32_t parent_cluster, const char name83[11],
...
int fat32_mount(FAT32_FS* fs, fat32_read_sectors_fn read_sectors,
                fat32_write_sectors_fn write_sectors, void* read_user, void* write_user,
    ...
    uint32_t data_sectors =
        total_sectors - (fs->reserved_sectors + (uint32_t)fs->fat_count * fs->sectors_per_fat);
...
int fat32_read(FAT32_File* file, void* out_buffer, uint32_t bytes_to_read,
...
int fat32_list_dir(FAT32_FS* fs, const char* path, fat32_list_callback_fn callback,
                   void* callback_user) {
    FAT32_DirEntry base;
    uint32_t cluster;
