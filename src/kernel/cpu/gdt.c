...
static void gdt_set_gate64(uint32_t i, uint64_t base, uint32_t limit, uint8_t access, uint8_t gran) {
...
void gdt_init(void) {
    ...
    // TSS descriptor occupies two entries in 64-bit GDT
    gdt_set_gate64(5, tss_base, tss_limit, 0x89, 0x00);
    gdt_set_gate64(6, (tss_base >> 32), 0, 0, 0); // upper part for TSS
