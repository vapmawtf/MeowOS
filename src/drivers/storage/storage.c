...
static IDE_Channel g_ide_channels[2] = { { IDE_PRIMARY_BASE, IDE_PRIMARY_CTRL },
...
static int atapi_send_packet(const IDE_Channel* channel, uint8_t drive, const uint8_t packet[12],
                             void* out, uint32_t out_bytes) {
...
static int ide_identify(uint8_t channel_index, uint8_t drive, uint32_t* out_sectors) {
    ...
    uint64_t sectors48 = ((uint64_t)identify[100]) | ((uint64_t)identify[101] << 16) |
...
static int ahci_exec(AHCI_Device* dev, uint8_t command, uint64_t lba, uint16_t sector_count,
...
static int ahci_identify(AHCI_Device* dev, uint32_t* out_sectors) {
    ...
    sectors48 = ((uint64_t)identify[100]) | ((uint64_t)identify[101] << 16) |
                ((uint64_t)identify[102] << 32) | ((uint64_t)identify[103] << 48);
...
static int ahci_read(void* user, uint32_t lba, uint32_t count, void* buffer) {
    ...
    while (done < count) {
        ...
        if (ahci_exec(dev, ATA_CMD_READ_DMA_EX, (uint64_t)lba + done, chunk,
                      (uint8_t*)buffer + ((size_t)done * 512u), 0) != 0) {
            return -1;
        }
...
static int ahci_write_dev(AHCI_Device* dev, uint32_t lba, uint32_t count, const void* buffer) {
    ...
    while (done < count) {
        ...
        if (ahci_exec(dev, ATA_CMD_WRITE_DMA_EX, (uint64_t)lba + done, chunk,
                      (void*)((const uint8_t*)buffer + ((size_t)done * 512u)), 1) != 0) {
            return -1;
        }
...
static void ide_init_devices(void) {
    ...
    for (uint8_t ch = 0; ch < 2; ch++) {
        for (uint8_t drv = 0; drv < 2; drv++) {
            ...
            if (vfs_register_block_device(dev->name, dev->sector_size, dev->sector_count, ide_read,
                                          is_atapi ? 0 : ide_write, dev) == 0) {
                if (is_atapi) {
                    printf("[storage] ATAPI %s: channel=%u drive=%u sectors=%u\n", dev->name, ch,
                           drv, dev->sector_count);
                } else {
...
static void ahci_probe_controller(uint32_t abar_phys) {
    ...
    for (uint8_t port = 0; port < 32 && g_ahci_device_count < AHCI_MAX_DEVICES; port++) {
        ...
        if (vfs_register_block_device(dev->name, 512, dev->sector_count, ahci_read, ahci_write,
                                      dev) == 0) {
            printf("[storage] AHCI %s: port=%u sectors=%u\n", dev->name, port, dev->sector_count);
            g_ahci_device_count++;
        } else {
            dev->present = 0;
        }
