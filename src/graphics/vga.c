...
static const uint8_t glyph_box[8] = { 0x7E, 0x42, 0x5A, 0x5A, 0x5A, 0x5A, 0x42, 0x7E };

static const uint8_t glyph_question[8] = { 0x3C, 0x42, 0x02, 0x0C, 0x10, 0x00, 0x10, 0x00 };
...
static uint32_t utf8_next_raw(const uint8_t** p, const uint8_t* end) {
    ...
    if ((b0 & 0xF8u) == 0xF0u) {
        if ((end - *p) < 3 || !is_utf8_cont((*p)[0]) || !is_utf8_cont((*p)[1]) ||
            !is_utf8_cont((*p)[2])) {
            return 0xFFFFFFFFu;
        }
        uint8_t b1 = *(*p)++;
